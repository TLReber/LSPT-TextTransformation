...
from text_transformation.transformer import title, strip, ngrams, Transformer
...
html2 = "<html><head><title></title></head><body>words words words</body></html>"
html3 = "<html></html>"
...
def test_transformer_ngrams():
    t = Transformer("ngrams")
    soup = BeautifulSoup(text1, 'html.parser')
    assert t(stop_words_list, [1, 2, 3], soup) == {1: {"hello": [0, 1], "world": [2]}, 2: {"hello hello": [0], "hello world": [1]}, 3: {"hello hello world": [0]}}
    soup = BeautifulSoup(text2, 'html.parser')
    assert t(stop_words_list, [1, 2, 3], soup) == {1: {"hello": [0, 1, 2]}, 2: {"hello hello": [0, 1]}, 3: {"hello hello hello": [0]}}
    soup = BeautifulSoup(text3, 'html.parser')
    assert t(stop_words_list, [1, 2, 3], soup) == {1: {}, 2: {}, 3: {}}
    soup = BeautifulSoup(text4, 'html.parser')
    assert t(stop_words_list, [1, 2, 3], soup) == {1: {}, 2: {}, 3: {}}
    soup = BeautifulSoup(html1, 'html.parser')
    assert t(stop_words_list, [1, 2, 3], soup) == {1: {"dormouse's": [1, 4], "story": [2, 5], "once": [6], "upon": [7], "time": [9]},
    2: {"dormouse's story" : [1, 4], "story once": [5], "once upon": [6]}, 3: {"dormouse's story once": [5], "story once upon": [6]}}
    soup = BeautifulSoup(html2, 'html.parser')
    assert t(stop_words_list, [1, 2, 3], soup) == {1: {"words": [0, 1, 2]}, 2: {"words words": [0, 1]}, 3: {"words words words": [0]}}
    soup = BeautifulSoup(html3, 'html.parser')
    assert t(stop_words_list, [1, 2, 3], soup) == {1: {}, 2: {}, 3: {}}
    soup = BeautifulSoup(html4, 'html.parser')
    assert t(stop_words_list, [1, 2, 3], soup) == {1: {}, 2: {}, 3: {}}
